 ...
 * Android 库模块通用配置插件
 * 
 * 该插件为 Android 库模块提供统一的构建配置，包括：
 * - 自动应用必要的 Android 库和 Kotlin 插件
 * - 统一的 SDK 版本配置
 * - 标准的构建类型配置
 * - Java 和 Kotlin 编译选项
 * - ProGuard 混淆规则配置
 ...
 * ```
 * 
 * 注意：该插件不包含 Compose 配置，如需 Compose 支持，请额外添加：
 * ```kotlin
 * plugins {
 *     id("android-library-convention")
        ...
        with(target) {
            ...
            extensions.configure<LibraryExtension> {
                ...
                defaultConfig {
                    ...
                    consumerProguardFiles("consumer-rules.pro") // 消费者 ProGuard 规则文件
                }
